const MoneyMarketHandler = artifacts.require("MoneyMarketHandler");
...
describe("MoneyMarketHandler contract", function () {
  ...
  async function resetContracts() {
    ...
    // Money Market
    money_market = await MoneyMarketMock.new();

    handler = await MoneyMarketHandler.new(
  ...
  describe("Deployment", function () {
    it("Should deployed and only initialized once", async function () {
      ...
      await truffleAssert.reverts(
        handler.initialize(dtoken_controller.address, money_market.address, {
          from: owner,
        }),
  ...
  describe("approve", function () {
    ...
    it("Should only allow auth to approve", async function () {
      ...
      allowance = await USDC.allowance(handler.address, money_market.address);
      assert.equal(allowance.toString(), UINT256_MAX.toString());
    ...
    it("Should fail if underlying approve failed", async function () {
      // Already approved when setting up, approve again would fail
      await truffleAssert.reverts(
