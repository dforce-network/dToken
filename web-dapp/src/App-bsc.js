...
export default class App extends React.Component {
  constructor(props) {
    ...
    this.state = {
      token_name: ['BUSD', 'USDT', 'USDC', 'DAI'],
      token_d_name: ['dBUSD', 'dUSDT', 'dUSDC', 'dDAI'],
      token_d_balance__prev: [0, 0, 0, 0],
      token_logo: [BUSD_logo, USDT_logo, USDC_logo, DAI_logo],
      token_decimals: [],
      token_contract: [],
      token_d_contract: [],
      token_is_approve: [],
  ...
  getUrl = (props) => {
    ...
    if (window.location.href.toLowerCase().includes('dapp-bsc/usdc')) {
      console.log('dapp-bsc/usdc');
      return 2;
    } else if (window.location.href.toLowerCase().includes('dapp-bsc/usdt')) {
      console.log('dapp-bsc/usdt');
      return 1;
    } else {
      console.log('dapp-bsc/busd');
      return 0;
    }
  ...
  componentDidMount = async () => {
    ...
    let netID = await this.new_web3.eth.net.getId();
    if (netID === 56) {
      this.setState({
        net_type: 'bsc',
      }, () => {
              ...
              <>
                {
                  ...
                  <a
                    ...
                    href={`https://bscscan.com/address/${this.state.my_account}`}
                  >
                    <div>
                      <i style={{ backgroundColor: 'rgb(235,172,14)' }}></i>
                          ...
                          <Menu className="menu">
                            <Menu.Item key={0}>
                              <div onClick={() => { this.set_mint_token(0) }}>
                                ...
                                <span className="span-name">BUSD</span>
