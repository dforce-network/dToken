...
contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    function accrueInterest() public returns (uint256) {
        ...
        vars.borrowRateMantissa = interestRateModel.getBorrowRate(
            cashPrior,
            totalBorrows,
            totalReserves
        );
        ...
        returns (uint256, uint256)
    {
        ...
        if (vars.err != Error.NO_ERROR) {
            return (
                fail(vars.err, FailureInfo.MINT_TRANSFER_IN_NOT_POSSIBLE),
                0
            );
        }
        ...
        returns (uint256, uint256)
    {
        ...
        if (err != Error.NO_ERROR) {
            return (
                fail(err, FailureInfo.ADD_RESERVES_TRANSFER_IN_NOT_POSSIBLE),
                actualAddAmount
            );
        }
